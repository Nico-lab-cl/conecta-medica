...
function Icon({
  ...
  /* Un export offline puede sustituir las URLs por datos embebidos poblando
     window.__CONECTA_ICONS["<set>:<name>"]. En línea no cambia nada. */
  const override = typeof window !== "undefined" && window.__CONECTA_ICONS && window.__CONECTA_ICONS[set + ":" + name];
  const url = override || (set === "simple" ? SIMPLE : LUCIDE) + name + ".svg";
